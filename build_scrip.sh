...
then
	...
	echo "		-d (String)	deploy mode (defualt prod)"
	exit 1
fi
...
ssh -i $CERTIFICATE_PATH -l $USER $HOST "PID=\$(ps -p \$(lsof -ti tcp:8080) o pid=); kill -15 \$PID; sleep 5; nohup java -Dspring.profiles.active=$DEPLOY -jar $LOCATION/$JAR_NAME > subway.out 2> subway.err < /dev/null &"
