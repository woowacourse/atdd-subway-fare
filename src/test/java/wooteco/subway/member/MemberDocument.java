...
public class MemberDocument extends Documentation {
    ...
    void duplicateEmail() {
        ...
        // when
        ExtractableResponse<Response> 이메일_중복_여부 = 이메일_중복_여부_확인_요청_시_실패(EMAIL);
    ...
    void newEmail() {
        ...
        // when
        ExtractableResponse<Response> 이메일_중복_여부 = 이메일_중복_여부_확인_요청_시_성공(NEW_EMAIL);

        // then
        assertThat(이메일_중복_여부.statusCode()).isEqualTo(HttpStatus.OK.value());
    }

    private ExtractableResponse<Response> 이메일_중복_여부_확인_요청_시_성공(String email) {
        return given("member/invalid-email/success")
                .when().get("/members/check-validation?email={email}", email)
                .then().log().all()
                .extract();
    }

    private ExtractableResponse<Response> 이메일_중복_여부_확인_요청_시_실패(String email) {
        return given("member/invalid-email/fail")
                .when().get("/members/check-validation?email={email}", email)
                .then().log().all()
                .extract();
    }
