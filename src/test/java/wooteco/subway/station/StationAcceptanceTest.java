...
import wooteco.subway.line.SectionAcceptanceTest;
...
public class StationAcceptanceTest extends AcceptanceTest {
    ...
    @DisplayName("잘못된 이름으로 지하철역을 생성한다.")
    @Test
    void createStationWithWrongName() {
        // when
        ExtractableResponse<Response> response = 지하철역_생성_요청("오류");
    ...
    @DisplayName("노선 위 지하철역을 제거한다.")
    @Test
    void deleteStationOnLine() {
        // given
        StationResponse stationResponseGangNam = 지하철역_등록되어_있음(강남역);
        StationResponse stationResponseYeokSam = 지하철역_등록되어_있음(역삼역);
        StationResponse stationResponse = 지하철역_등록되어_있음(선릉역);
        LineResponse 신분당선 = 지하철_노선_등록되어_있음("신분당선", "bg-red-600", stationResponseGangNam,
            stationResponseYeokSam, 10);
        SectionAcceptanceTest.지하철_구간_생성_요청(신분당선, stationResponseYeokSam, stationResponse, 10);
    ...
    public static ExtractableResponse<Response> 노선_위_지하철역_제거_요청(StationResponse stationResponse) {
        return RestAssured
            .given().log().all()
            .when().delete("/stations/" + stationResponse.getId())
            .then().log().all()
