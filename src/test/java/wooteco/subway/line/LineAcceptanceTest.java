...
public class LineAcceptanceTest extends AcceptanceTest {
    ...
    private ExtractableResponse<Response> findLineList(String token) {
        return RestAssured.given().log().all()
                .auth().oauth2(token)
                .accept(MediaType.APPLICATION_JSON_VALUE)
                .when().get("/lines")
    ...
    private ExtractableResponse<Response> editLine(long id, LineRequest lineRequest, String token) {
        return RestAssured.given().log().all()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .body(lineRequest)
                .auth().oauth2(token)
    ...
    private ExtractableResponse<Response> deleteLine(long id, String loginToken) {
        return RestAssured.given().log().all()
    ...
    public void setUp() {
        ...
        firstStation = createStation("강남역", loginToken)
                .as(StationResponse.class);
        secondStation = createStation("광교역", loginToken)
                .as(StationResponse.class);
    ...
    void findOneLine() throws JsonProcessingException {
        LineResponse lineResponse = createLine(firstLineRequest, loginToken)
    ...
    @Test
    void findLineList() {
        LineResponse firstLineResponse = createLine(firstLineRequest, loginToken)
                .as(LineResponse.class);
        LineResponse secondLineResponse = createLine(secondLineRequest, loginToken)
                .as(LineResponse.class);

        ExtractableResponse<Response> response = findLineList(loginToken);
    ...
    void cannotEditLineWhenIdInvalid() {
        ...
        ExtractableResponse<Response> response = editLine(999, changeRequest, loginToken);
    ...
    void cannotEditLineAsDuplicatedName() {
        ...
        ExtractableResponse<Response> response = editLine(lineResponse.getId(), changeRequest, loginToken);
    ...
    void cannotDeleteLineWhenInvalidId() {
        ExtractableResponse<Response> response = deleteLine(370, loginToken);
    ...
    void deleteLine() {
        ...
        ExtractableResponse<Response> response = deleteLine(lineResponse.getId(), loginToken);
