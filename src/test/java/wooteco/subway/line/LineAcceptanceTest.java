...
public class LineAcceptanceTest extends AcceptanceTest {
    ...
    void getLinesDetail() {
        ...
        // when
        ExtractableResponse<Response> response = 지하철_노선_상세목록_조회_요청();

        // then
        지하철_노선_목록_응답됨(response);
        지하철_노선_상세목록_포함됨(response, Arrays.asList(lineResponse1, lineResponse2));
    }
    ...
    void getLineDetail() {
        ...
        // when
        ExtractableResponse<Response> response = 지하철_노선_상세조회_요청(lineResponse);

        // then
        지하철_노선상세_응답됨(response, lineResponse);
    }
    ...
    private static ExtractableResponse<Response> 지하철_노선_상세목록_조회_요청() {
        return RestAssured
                .given().log().all()
                .accept(MediaType.APPLICATION_JSON_VALUE)
    ...
    public static ExtractableResponse<Response> 지하철_노선_상세조회_요청(LineResponse response) {
        return RestAssured
                .given().log().all()
                .accept(MediaType.APPLICATION_JSON_VALUE)
    ...
    public static void 지하철_노선상세_응답됨(ExtractableResponse<Response> response, LineResponse lineResponse) {
        assertThat(response.statusCode()).isEqualTo(HttpStatus.OK.value());
    ...
    public static void 지하철_노선_상세목록_포함됨(ExtractableResponse<Response> response, List<LineResponse> createdResponses) {
        List<Long> expectedLineIds = createdResponses.stream()
                .map(it -> it.getId())
                .collect(Collectors.toList());
