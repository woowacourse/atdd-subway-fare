...
import static wooteco.subway.line.LineAcceptanceTest.지하철_추가요금노선_등록되어_있음;
...
public class PathAcceptanceTest extends AcceptanceTest {
    ...
    public void setUp() {
        ...
        신분당선 = 지하철_추가요금노선_등록되어_있음("신분당선", "bg-red-600", 강남역, 양재역, 10, 500);
        이호선 = 지하철_노선_등록되어_있음("이호선", "bg-green-600", 교대역, 강남역, 10);
        삼호선 = 지하철_노선_등록되어_있음("삼호선", "bg-blue-600", 교대역, 양재역, 5);

        지하철_구간_등록되어_있음(삼호선, 교대역, 남부터미널역, 3);
    }

    @DisplayName("두 역의 최단 거리 경로 및 요금을 조회한다.")
    @Test
    ...
    @DisplayName("노선 추가요금이 계산되는지 검증한다.")
    @Test
    ...
    private static void 요금이_응답됨(ExtractableResponse<Response> response, int fare) {
        PathResponse pathResponse = response.as(PathResponse.class);
